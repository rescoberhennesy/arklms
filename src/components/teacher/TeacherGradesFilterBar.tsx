...
interface TeacherGradesFilterBarProps {
  /** All of the teacher's classes (active only) — used to derive dropdown options. */
  classes: TeacherClassListItem[];
  /** How many classes are currently visible after filtering. */
  matchedCount: number;
  /** Total classes the teacher owns (pre-filter). */
  totalCount: number;
}

/**
 * Filter bar for the aggregated /teacher/grades page.
 *
 * All four filters live in the URL (?classId=&section=&track=&grade=) so the
 * server component owns data fetching and the page is refresh-safe. The class
 * picker pins to one class — it shadows the other three filters when set,
 * which we surface visually by disabling them.
 */
export default function TeacherGradesFilterBar({
  classes,
  matchedCount,
  totalCount,
}: TeacherGradesFilterBarProps) {
  ...
  // Section options: distinct non-null sections from the teacher's classes.
  ...
  function setParam(key: string, value: string) {
    ...
    // When you pick a specific class, the other filters become noise — clear
    // them so the URL reflects the actual state.
